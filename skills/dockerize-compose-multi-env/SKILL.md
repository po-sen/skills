...
1. Inspect the repo to infer stack and runtime.
   ...
   - List top-level files and check for existing `Dockerfile`, `docker-compose*.yml`, `.env*`,
     README.
...
2. Infer dependent services and env vars.
   ...
   - Search for DB/cache/queue/object storage usage and env vars (e.g., `DATABASE_URL`,
     `POSTGRES_*`, `REDIS_URL`, `BROKER_URL`, `S3_*`).
...
3. Ask mandatory Q&A before generating files.
   ...
   - Confirm which env files should exist per environment (default: `.env.production`,
     `.env.staging`, `.env.test`) and whether any already exist.
...
4. Create or update `.dockerignore`.
   ...
   - Use a conservative baseline (see `references/dockerignore.md`) and remove any ignores that
